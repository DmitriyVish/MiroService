...
# #  ---------- Общие списки ----------
# all_criteria = [criteria_q1, criteria_q2, criteria_q3, criteria_q4]
# max_scores = [len(crit) for crit in all_criteria]  # [5,5,10,4]

# # ---------- Функции загрузки и оценки ----------
...
#         if crit['name'] == 'Отсутствие ошибок':
#             found = check_keywords(answer, crit['keywords'])
...
#         if 1 <= num <= 4:
#             answers[num-1] = content
#     return answers, data.get('filename', os.path.basename(filepath))

# # ---------- Получаем все JSON-файлы в текущей папке и сортируем по имени ----------
# json_files = glob.glob('*.json')
# json_files.sort()  # сортировка по имени (как строки)

# print(f"\n{'='*60}")
# print(f"Найдено JSON-файлов: {len(json_files)}")
# print("Файлы для обработки:", json_files)

# total_candidates = 0
# for file_name in json_files:
#     # Можно пропустить сам скрипт, если он сохранён как .json (маловероятно)
#     if not os.path.isfile(file_name):
#         continue
    
#     answers, fname = load_answers_from_file(file_name)
#     print(f"\n{'='*60}")
#     print(f"Оценка для файла: {fname}")
#     print('='*60)
...
#         criteria = all_criteria[q_idx]   # предполагается, что all_criteria определён
#         max_score = max_scores[q_idx]    # и max_scores тоже
            
#         if answer is None or len(answer.strip()) < 5:
#             print(f"\nВопрос {q_idx+1}: ответ отсутствует или слишком короткий (0 из {max_score})")
#             continue
        
#         results, score = evaluate_answer(answer, criteria)
#         total_score_all += score
#         total_max_all += max_score
        
#         print(f"\n--- Вопрос {q_idx+1} (макс. оценка {max_score}) ---")
#         answer_preview = answer.strip()[:500] + "..." if  len(answer) > 500 else answer.strip()
#         print("Ответ:", answer_preview)
#         print(f"\n{'-'*60}")
#         print("Результаты по критериям:")
#         for crit, res in zip(criteria, results):
#             status = "✓" if res else "✗"
#             print(f"  {status} {crit['name']}")
#         print(f"Итого за {q_idx+1} вопрос: {score} из {max_score}")
    
#     print(f"\n{'-'*60}")
#     print(f"\n*** ОБЩИЙ БАЛЛ для {fname}: {total_score_all} из {total_max_all} ***")
    
#     total_candidates += 1

# print(f"\nОбработано кандидатов: {total_candidates}")